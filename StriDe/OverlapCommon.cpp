    ...
    for(size_t i = 0; i < numBlocks; ++i)
    {
        ...
        for(int64_t j = record.ranges.interval[0].lower; j <= record.ranges.interval[0].upper; ++j)
        {
            ...
            {    
				...
				DenseHashSet <std::string,StringHasher>::const_iterator found  = SuperRepeatVertices->find(o.id[1]);
				if (found != SuperRepeatVertices->end() && o.match.getMinOverlapLength()<81)
					continue;
