...
MultipleAlignment KmerOverlaps::buildMultipleAlignment(const std::string& query,
													   ...
													   KmerContext& kc)
{
    SequenceOverlapPairVector overlap_vector = retrieveMatches(query, k, min_overlap, min_identity, kmerThreshod, indices, erroridx,kc);
...
SequenceOverlapPairVector KmerOverlaps::retrieveMatches(const std::string& query,
														...
														KmerContext& /*kc*/)
{
    PROFILE_FUNC("KmerOverlaps::retrieveMatches")
    ...
    int64_t max_interval_size = 50; //larger->slower but align more
    ...
    size_t i=erroridx;	//erroridx is not always precise, shift left 5bp for safety
    for(; i < num_kmers; i++)
    {
		std::string kmer = query.substr(i, k);
		BWTInterval interval = BWTAlgorithms::findInterval(indices, kmer);

		// std::cout << i << ":\t" << query << "\t"<<interval.size();
		if(interval.isValid() && interval.size()>=kmerThreshold)
		{
			for(int64_t j = interval.lower; j <= interval.upper && (j-interval.lower < max_interval_size); ++j)
			{
		...
		//if(prematchMap.size()> (size_t)max_interval_size*2*20) break;
    }
    ...
    for(KmerMatchMap::iterator iter = prematchMap.begin(); iter != prematchMap.end(); ++iter)
    {
		...
		//if(matches.size()> (size_t)max_interval_size*2) break;
    }

	// std::cout << prematchMap.size() << "\t";
	// std::cout << matches.size() << "\t" << matches.size()/(double)prematchMap.size() << "\n";
    ...
    int64_t maxAlignSeq=0;
    for(KmerMatchSet::iterator iter = matches.begin(); iter != matches.end()
        && maxAlignSeq<= max_interval_size; ++iter)
    {
		...
		size_t pos_0 = iter->position;
        size_t pos_1 = match_sequence.find(match_kmer);
        assert(pos_0 != std::string::npos && pos_1 != std::string::npos);

		//assume gaps occupy at most half of the misaligned positions 
        size_t bandwidth=query.length()*(1-min_identity);
        ...
        if(query.find(match_kmer, pos_0 + 1) != std::string::npos ||
           match_sequence.find(match_kmer, pos_1 + 1) != std::string::npos) {
            // One of the reads has a second occurrence of the kmer. Use the slow overlapper.
