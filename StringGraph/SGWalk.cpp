...
int SGWalk::getEndToStartDistance(Vertex* pEnd) const
{
	if(m_edges.empty()) return 0;
	
	size_t i = 0 , distanceStartToEnd = m_pStartVertex->getSeqLen();
	for (i=0 ;i<m_edges.size();i++)
	{
		distanceStartToEnd += m_edges[i]->getSeqLen();
		if (m_edges[i]->getEnd()==pEnd) break;
	}
	if (i==m_edges.size()) return 0 ;
	
	int len_x = m_pStartVertex->getSeqLen();
	int len_y = m_edges[i]->getEnd()->getSeqLen();
	
	return distanceStartToEnd - (len_x + len_y);
