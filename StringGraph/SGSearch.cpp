...
//
void SGSearch::getTree(Vertex* root ,EdgeDir initialDir,int maxDistance,size_t maxNodes,SGWalkVector& outWalks)
{
	 SGSearchTree searchTree(root, NULL, initialDir, maxDistance, maxNodes);
	 while(searchTree.stepOnce()) { }
	 SGWalkBuilder builder(outWalks, false);
	 searchTree.buildWalksToAllLeaves(builder);
}
                                ...
                                SGWalkVector& outWalks)
{
	...
	findCollapsedWalks(pX, initialDir, maxDistance, maxWalks, outWalks);	//bug fixed by YTH, no pass of maxWalks
