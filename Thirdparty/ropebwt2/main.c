...
int main_ropebwt2(int argc, char *argv[])
{
	...
	if (m && buf.l) {
		double ct = cputime(), rt = realtime();
		int64_t numOfStrings= mr_insert_multi(mr, buf.l, (uint8_t*)buf.s, flag&FLAG_THR);
		if (verbose >= 3) fprintf(stderr, "[M::%s] inserted %ld symbols in %.3f sec, %.3f CPU sec from %ld strings\n",
				__func__, (long)buf.l, realtime() - rt, cputime() - ct, numOfStrings);
	}
	if (verbose >= 3) {
		int64_t c[6];
